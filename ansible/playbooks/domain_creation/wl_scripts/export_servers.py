...
admin_nm = cnf.admin_name
dmn_hm = cnf.domain_home

sec_d_l = dmn_hm +r"/servers/"+admin_nm+r"/"
...
# Here we will eliminate the admin server if it is present in managed server list

for item in unq_mng_srv:
	if(item == admin_srv):
		unq_mng_srv.remove(item)
	else:
		continue
...
var_file.write("boot_identity_location: "+sec_d_l+"\n")
var_file.write("...\n")
